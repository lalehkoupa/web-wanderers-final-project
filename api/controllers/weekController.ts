...
weeksRouter
  .get("/", async (req, res) => {
    try {
      ...
      if (!allWeeks) {
        res.status(404).json({ error: true, msg: "Cannot find any week " });
      }
  ...
  .get("/:id", async (req, res) => {
    try {
      ...
      if (!week) {
        return res
          .status(404)
          .json({ error: true, msg: "Cannot find this week :(" });
        return;
      }
    ...
    try {
      ...
      if (weekExists) {
        res
          .status(400)
          .json(
            "This week already exists! Please check the params and or week date"
          );
        return;
      }
